...
import 'package:zd_flutter_utils/time/abs_time_info.dart';
import 'package:zd_flutter_utils/time/day_format.dart';
import 'package:zd_flutter_utils/time/en_info_impl.dart';
import 'package:zd_flutter_utils/time/zh_normal_info_impl.dart';

import 'en_normal_info_impl.dart';
import 'zh_info_impl.dart';
...
class TimeUtils {
  ...
  static String format(
    ...
    if (_info.customYesterday().isNotEmpty &&
        ZdDateUtils.isYesterdayByMilliseconds(ms, _locTimeMs)) {
      return _getYesterday(ms, _info, _dayFormat);
    }

    if (!ZdDateUtils.yearIsEqualByMilliseconds(ms, _locTimeMs)) {
  ...
  static String formatA(
    ...
    int elapsed = _locTimeMs - ms;
    if (elapsed < 0) {
      return ZdDateUtils.formatDateMilliseconds(ms, format: formatToday);
    }

    if (ZdDateUtils.isToday(ms, locMs: _locTimeMs)) {
      return ZdDateUtils.formatDateMilliseconds(ms, format: formatToday);
    }

    if (ZdDateUtils.isYesterdayByMilliseconds(ms, _locTimeMs)) {
      return languageCode == 'zh' ? '昨天' : 'Yesterday';
    }

    if (ZdDateUtils.isWeek(ms, locMs: _locTimeMs)) {
      return ZdDateUtils.getWeekdayByMilliseconds(ms,
          languageCode: languageCode, short: short);
    }
    return ZdDateUtils.formatDateMilliseconds(ms, format: format);
  }
  ...
  static String _getYesterday(
    ...
    return info.customYesterday() +
        (dayFormat == DayFormat.Full
            ? (' ' + ZdDateUtils.formatDateMilliseconds(ms, format: 'HH:mm'))
            : '');
  }
  ...
  static String _getYear(
    ...
    if (dayFormat != DayFormat.Simple) {
      return ZdDateUtils.formatDateMilliseconds(ms,
  ...
  static String _formatDays(
    ...
    switch (dayFormat) {
      ...
      case DayFormat.Common:
        timeline = ZdDateUtils.formatDateMilliseconds(ms, format: 'MM-dd');
        break;
      case DayFormat.Full:
        timeline =
            ZdDateUtils.formatDateMilliseconds(ms, format: 'MM-dd HH:mm');
        break;
    }
    return timeline;
  }
