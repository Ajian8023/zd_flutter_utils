...
class ZdDateUtils {
  ...
  static bool isToday(int milliseconds, {bool isUtc = false, int? locMs}) {
    ...
    DateTime now;
    if (locMs != null) {
      now = ZdDateUtils.getDateTimeByMs(locMs);
    } else {
  ...
  static bool isWeek(int ms, {bool isUtc = false, int? locMs}) {
    ...
    DateTime _now;
    if (locMs != null) {
      _now = ZdDateUtils.getDateTimeByMs(locMs, isUtc: isUtc);
    } else {
  ...
  static DateTime addMonths(DateTime date, int months) {
    var res = ZdDateUtils.copyWith(date, month: date.month + months);
    if (date.day != res.day) res = ZdDateUtils.copyWith(res, day: 0);
    return res;
  }
  ...
  static int getWeekNumber(DateTime date, {int? firstWeekday}) {
    ...
    if (isWeekInYear(date, date.year, firstWeekday ?? DateTime.monday)) {
      final startOfTheFirstWeek = firstDayOfFirstWeek(date.year,
          firstWeekday: firstWeekday ?? DateTime.monday);
  ...
  static int getLastWeekNumber(int year, {int? firstWeekday}) {
    ...
    final start = firstDayOfFirstWeek(year,
        firstWeekday: firstWeekday ?? DateTime.monday);
    ...
    if (isWeekInYear(end, year, firstWeekday ?? DateTime.monday)) res++;
    return res;
  }
  ...
  static bool isFirstDayOfWeek(DateTime day, {int? firstWeekday}) {
    ...
    return isSameDay(
        firstDayOfWeek(day, firstWeekday: firstWeekday ?? DateTime.monday),
        day);
  }
  ...
  static bool isLastDayOfWeek(DateTime day, {int? firstWeekday}) {
    ...
    return isSameDay(
        lastDayOfWeek(day, firstWeekday: firstWeekday ?? DateTime.monday), day);
  }
  ...
  static DateTime firstDayOfFirstWeek(int year, {int? firstWeekday}) {
    ...
    return isWeekInYear(startOfYear, year, firstWeekday ?? DateTime.monday)
        ? firstDayOfWeek(startOfYear, firstWeekday: firstWeekday)
        : firstDayOfNextWeek(startOfYear,
            firstWeekday: firstWeekday ?? DateTime.monday);
  }
