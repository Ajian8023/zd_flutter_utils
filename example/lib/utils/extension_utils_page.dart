...
import 'package:zd_flutter_utils/extens/extension_int.dart';
import 'package:zd_flutter_utils/extens/extension_list.dart';
...
class _DatePageState extends State<ExtensionPage> {
  ...
  int a1 = 1881;
  int a2 = 25;
  List<dynamic> list = new List.filled(3, 0, growable: true);
  Map map = new Map();
    ...
    return Scaffold(
      ...
      body: new Column(
        children: <Widget>[
          ...
          new Text("将数字字符串转int：" +
              NumUtils.getIntByValueString("yangchong").toString()),
          new Text("数字字符串转double：" +
              NumUtils.getDoubleByValueString(str2).toString()),
          new Text("数字保留x位小数：" +
              NumUtils.getNumByValueString(str2, fractionDigits: 2).toString()),
          new Text(
              "浮点数字保留x位小数：" + NumUtils.getNumByValueDouble(d, 2).toString()),
          new Text(
              "浮点数字保留x位小数：" + NumUtils.getNumByValueDouble(d, 2).toString()),
