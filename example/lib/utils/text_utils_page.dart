...
import 'package:zd_flutter_utils/text/text_utils.dart';
    ...
    return Scaffold(
      ...
      body: new Column(
        children: <Widget>[
          ...
          new Text(
              "判断字符串是以ya开头：" + TextUtils.startsWith(string, "ya").toString()),
          new Text(
              "判断字符串是否包含ya：" + TextUtils.contains(string, "ya").toString()),
          new Text("判断字符串是否包含ya：" + TextUtils.abbreviate(string, 4).toString()),
          new Text("判断字符串是否包含ya：" + TextUtils.abbreviate(string, 7).toString()),
          new Text(
              "比较两个字符串是否相同：" + TextUtils.compare(string, "hah").toString()),
          new Text("比较两个字符串是否相同：" + TextUtils.compare("hah", "hah").toString()),
          new Text("比较两个字符串是否相同：" +
              TextUtils.hammingDistance("yangchong", "chongyang").toString()),
          new Text("格式化银行卡：" + TextUtils.formatDigitPattern(card).toString()),
          new Text("格式化手机号，从后面开始：" +
              TextUtils.formatDigitPatternEnd(phone).toString()),
          ...
          new Text("隐藏手机号中间n位：" +
              TextUtils.hideNumber(phone, start: 2, end: 5).toString()),
